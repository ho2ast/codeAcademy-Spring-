	...
	<div class="row">
		...
		<div class="col-md-12">
			...
			<c:if test="${pageInfoMap.startPage gt pageInfoMap.pageBlock }">
				<a href="/community/qna?pageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock + 4}&search=${search}">[이전]</a>
			</c:if>
			
			<c:forEach var="i" begin="${pageInfoMap.startPage}" end="${pageInfoMap.endPage}" step="1">
				<a href="/community/qna?pageNum=${i}&search=${search}">
				<c:choose>
			...
			<c:if test="${pageInfoMap.endPage lt pageInfoMap.pageCount}">
				<a href="/community/qna?pageNum=${pageInfoMap.startPage - pageInfoMap.pageBlock + 4}&search=${search}">[다음]</a>
			</c:if>
		</div>
		
		<form action="/community/qna" method="get">
			<div id="board_search">
				<c:choose>
